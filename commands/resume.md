...
Check if `.claude/autoresearch-loop.local.md` exists.

**If it exists:** Update the `session_id` field to the current session so the stop hook recognizes this session:

```bash
sed -i '' "s/^session_id: .*/session_id: ${CLAUDE_CODE_SESSION_ID}/" .claude/autoresearch-loop.local.md
```

**If it doesn't exist:** Reconstruct it from `autoresearch.md`. Read `autoresearch.md` to extract the configuration (goal, metric, direction, scope), and count the lines in `autoresearch.jsonl` to determine the current iteration number. Then create `.claude/autoresearch-loop.local.md` with the correct state — use the same format as the setup script would produce.

## Step 3: Show current state

Read `autoresearch.md` and `autoresearch.jsonl`. Print a brief summary:
- Goal and metric
