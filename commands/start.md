...
allowed-tools: ["Bash(${CLAUDE_PLUGIN_ROOT}/scripts/setup-autoresearch.sh:*)", "Read", "Write", "Bash", "Glob", "Grep", "Edit"]
---
...
You MUST run the setup script below using the Bash tool. Do NOT skip this step. Do NOT create the session files manually. The setup script creates a state file (.claude/autoresearch-loop.local.md) that drives the automatic loop — without it, the loop will not continue between iterations.
...
After the script runs, verify that `.claude/autoresearch-loop.local.md` exists. If it does not, something went wrong — diagnose and fix before proceeding.

## Step 2: Run the research cycle

You are now in an autonomous research loop. Follow this cycle for EVERY iteration:

1. **Read state**: Read `autoresearch.md` to understand prior results, dead ends, and successful strategies.
...
5. **Measure**: Run `bash autoresearch.sh` and parse the `METRIC name=value` output line(s).
6. **Check** (if `autoresearch.checks.sh` exists): Run `bash autoresearch.checks.sh`. If it fails, the experiment fails regardless of metric improvement.
7. **Log**: Append one JSON line to `autoresearch.jsonl`:
   ```
   ...
   ```
8. **Decide**:
   - If improved AND checks passed: **keep**. Update `autoresearch.md` with what worked.
   - Otherwise: `git revert HEAD --no-edit`. Update `autoresearch.md` with what didn't work and why.
...
- Update `autoresearch.md` every iteration — it's your memory across context resets.
